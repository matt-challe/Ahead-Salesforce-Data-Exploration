create or replace view INTERNAL_SALESFORCE_DATA_ANALYTICS.RESOURCE_ANALYTICS.RESOURCE_HELD_FOR_LOST_OPPS(
	AHEAD_PRACTICE,
	YEAR_OPP_CLOSED,
	QTR_OPP_CLOSED,
	NUM_RESOURCES_HELD_AND_OPP_LOST,
	NUM_OPPS_LOST_W_RESOURCES_HELD
) as 
WITH staging AS ( 
    SELECT 
    ...
    FROM SALESFORCE_DATABASE.SALESFORCE.pse_resource_request_c AS pse_req
    LEFT OUTER JOIN SALESFORCE_DATABASE.SALESFORCE.opportunity AS opp ON pse_req.pse_opportunity_c = opp.id 
    LEFT OUTER JOIN SALESFORCE_DATABASE.SALESFORCE.pse_resource_request_history AS pse_history ON pse_history.parent_id = pse_req.id
    WHERE is_won = FALSE AND is_closed = TRUE 
        AND year_opp_closed >= 2021 AND opp_closed_date <= CURRENT_DATE()
        -- change above to: DATEDIFF(YEAR, opp_close_date, CURRENT_DATE()) <= 3 AND DATE_PART(YEAR, opp_close_date) >= 2021 AND opp_close_date <= CURRENT_DATE()
        -- will create a 3 year moving window
        AND resource_id IN (SELECT DISTINCT(pse_resource_c) FROM SALESFORCE_DATABASE.SALESFORCE.pse_resource_actuals_c)
